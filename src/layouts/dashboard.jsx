...
export default function DashboardLayout({ children, sidebar = true }) {
  ...
  return (
    <div className="flex flex-col h-svh overflow-hidden">
      ...
      >
        <section
          className="flex items-center justify-start py-2 flex-1 pr-3 h-full min-w-0"
          data-tauri-drag-region
        >
          <div
            className="flex flex-1 flex-row gap-2 overflow-x-auto min-w-0"
            data-tauri-drag-region
          >
            {TABS_ITEMS.map((item, index) => {
              return (
                <button
                  key={index}
                  className="text-xs px-2 pt-2 pb-1 mt-2.5 border-b-2 bg-card leading-none rounded-t-sm flex flex-row items-center justify-center gap-1 data-[active=true]:bg-primary data-[active=true]:text-primary-foreground cursor-pointer border-primary dark:border-muted  border data-[active=false]:border-b-background dark:data-[active=false]:border-b-sidebar shrink-0"
