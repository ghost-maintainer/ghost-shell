...
import { DownloadIcon, CheckCircle2, AlertTriangle, ShieldCheck } from "lucide-react";
...
export default function ExportData() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      setStatus({
        type: "error",
        message: typeof err === "string" ? err : err?.message || "Failed to export data.",
      });
    } finally {
      setLoading(false);
    }
      ...
      <div className="max-w-2xl mx-auto mt-8 space-y-6">
        <div className="border bg-sidebar rounded-xl p-6 shadow-sm space-y-4">
          <div className="flex items-center gap-3 border-b pb-4">
            ...
            <div>
              <h2 className="text-lg font-semibold text-foreground leading-none">Export Secure Vault</h2>
          ...
          <div className="space-y-4 py-2">
            <div className="flex flex-col gap-2 bg-muted p-4 rounded-lg">
              <div className="flex items-center gap-2 text-primary">
                ...
                <span className="text-sm font-semibold">Automatic Session Encryption</span>
              </div>
              <p className="text-xs leading-relaxed text-muted-foreground mt-1">
                The exported backup file is always encrypted using the secure key derived from your current login passphrase.
                No additional passwords are required to export or import this backup file.
              </p>
            </div>
          </div>
